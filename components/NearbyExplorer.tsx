...
export default function NearbyExplorer() {
  ...
  const { coords, city, country, refresh, error, loading } = useLocation();
  ...
  function quick(p: string) { setAutoMsg(p); setChatKey(k => k + 1); }
...
2. For EVERY place, include BOTH a Google Maps link and a Waze link in this exact markdown format:
   ...
   Replace PLACE_NAME and CITY with actual values, URL-encoded with + for spaces.
3. Format each place as: **Name** — short description, then both links.
4. Use bullet points. No long preamble.
5. Reply in ${lang === 'he' ? 'Hebrew (עברית)' : 'English'}.`;

  const initialMsg = coords
    ? (lang === 'he' ? `אני יודע שאתם ב**${locName || 'מיקום שלכם'}**. בחרו קטגוריה מהירה למטה או כתבו לי מה אתם מחפשים — אני אביא שמות אמיתיים עם לינקים לגוגל מפס ולוויז.` : `You're in **${locName || 'your location'}**. Pick a category below or tell me what you're looking for — I'll give real place names with Google Maps and Waze links.`)
    : (lang === 'he' ? 'מאתר את המיקום שלכם…' : 'Locating you…');
    ...
    <div className="space-y-4">
      <div className="card flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium">📍 {loading ? (lang === 'he' ? 'מאתר…' : 'Locating…') : (locName || (coords ? `${coords.lat.toFixed(3)}, ${coords.lng.toFixed(3)}` : (lang === 'he' ? 'מיקום לא ידוע' : 'unknown')))}</span>
        </div>
        <button onClick={refresh} className="btn-ghost !py-2 text-sm">{t('road.refresh')}</button>
        {error && <span className="w-full text-xs text-red-600">{error}</span>}
      </div>

      <div className="flex flex-wrap gap-2">
        {prompts.map(p => (
          <button key={p.label} onClick={() => quick(p.prompt)} disabled={!coords} className="rounded-full bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 ring-1 ring-slate-200 transition hover:bg-brand-50 hover:ring-brand-200 disabled:opacity-50">
            {p.label}
          </button>
        ))}
      </div>
